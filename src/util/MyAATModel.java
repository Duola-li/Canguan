...
public class MyAATModel extends AbstractTableModel {//居然是继承的这个，我草我草！！！！！//这个和之前的那个比，就是不让编辑

    private Vector TableTitle;//表格的 列标题
	private Vector TableData;//用来存放表格数据的线性表    
    public MyAATModel(Vector vname, Vector dd ) {
...
//    	}
	...
	{
//		if (columnIndex > TableTitle.size()-1 )
//   			return null;
	...
	public String getColumnName(int column)
	{
		return (String)TableTitle.get(column);//[column];
	       ...
	       public Object getValueAt(int rowIndex, int columnIndex)
	       {
	              ...
	              //例如
//	    	   		if (rowIndex > TableData.size()-1 || columnIndex > TableTitle.size()-1 )
//	    	   			return null;
...
//					return false;
	       ...
	       {               
	    	   RetrieveObject RO = new RetrieveObject();
	    	   	MyAATModel myModel=  RO.getTableModel(Define.Cai_columnNames, "select * from menu");
//	    	   Vector vname = new Vector(), dd=null;
//	    	   for (int i = 0 ; i < Define.Cai_columnNames.length ; i++){
//	               vname.addElement(Define.Cai_columnNames[i]);
//	           }
//	    	   MyAATModel myModel = new MyAATModel(vname, dd);
