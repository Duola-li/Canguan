...
public class JF_Waiter extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					JF_Waiter frame = new JF_Waiter(20001);
	 ...
	 * Create the frame.
	 */
	public JF_Waiter(final int Num) {
		sqlsql = "select Tnum, Cnum, Knum, send, made from consume where Wnum ="+Num;

		setTitle("侍者上菜服务管理("+Num+")");
		...
		OnlyNodone = new JRadioButton("只显示未上菜");
		...
		Button_search.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				 ...
				 * sql
				 */
				model_consume = RO.getTableModel(Define.Consume_columnNamesW, sqlsql+IsTable+IsOnlyDone);					//该滚滚滚滚滚滚滚滚
		...
		JButton Button_done = new JButton("完成上菜");
		Button_done.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if(Dealrow >= 0){
					int send = (int) table.getValueAt(Dealrow, Define.SEND);
					if(send > 0){
						JOptionPane.showMessageDialog(null,"对不起，已经上菜！","错误",JOptionPane. INFORMATION_MESSAGE);
						return;
					}
					int i=JOptionPane.showConfirmDialog(null, "确定完成此菜品？","提示", JOptionPane.YES_NO_OPTION);
	  	  			if(i==JOptionPane.OK_OPTION){
	  	  			table.setValueAt(1, Dealrow, Define.SEND);
	  	  			int made = (int) table.getValueAt(Dealrow, Define.MADE);
	  	  			if(made < 1){
						JOptionPane.showMessageDialog(null,"对不起，未完成菜品！","错误",JOptionPane. INFORMATION_MESSAGE);

	  	  			}else{
	  	  				
	  	  				int Cnum = (int) table.getValueAt(Dealrow, Define.Consume_Cnum);
	  	  				int Tnum = (int) table.getValueAt(Dealrow, Define.Consume_Tnum);
	  	  				ComonManage.setSend(Tnum, Cnum, Num, 1);
	  	  				Dealrow = -1;
	...
	//				//panel_1.setLayout(new BorderLayout(0, 0));
		 ...
		 * 表格
		 */
		model_consume = RO.getTableModel(Define.Consume_columnNamesW, sqlsql);					//该滚滚滚滚滚滚滚滚
		sorter = new TableRowSorter(model_consume);  					//该滚滚滚滚滚滚滚滚
		table = new JTable();
		table.setCellSelectionEnabled(false);//设置此表是否允许同时存在行选择和列选择。
		ListSelectionModel cellSelectionModel = table.getSelectionModel();
