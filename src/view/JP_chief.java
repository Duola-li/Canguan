...
public class JP_chief extends JPanel {
	...
	public JP_chief() {
		...
		model_chief = RO.getTableModel(Define.Chief_columnNames, sqlsql);
		sorter = new TableRowSorter(model_chief);  		
		table.setCellSelectionEnabled(false);//设置此表是否允许同时存在行选择和列选择。
		table.setModel(model_chief);
		...
		comboBox_level = new JComboBox(Define.Chieflevel);
		...
		insert.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int getCnum;
				try {
					getCnum = ComonManage.Max_num("chief")+1;
		...
		saveinsert.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int price = 0;
				int num;
				int intlevel = Obj_Chief.Tointlevel(level);
				try {	
				...
				int i=JOptionPane.showConfirmDialog(null, "确定插入？","提示", JOptionPane.YES_NO_OPTION);
  	  			if(i==JOptionPane.OK_OPTION){
  	  				 ...
  	  				 * ！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
  	  				 */
	  	  			Obj_Chief cai = new Obj_Chief(num, name, intlevel, price);
		  				try {
						ComonManage.Chief_manage(cai, ComonManage.Insert);
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
		...
		drop.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int num = 0;				
				String name = (String) table.getValueAt(Dealrow, Define.Model_Chiefname);
				try {
					num = Integer.valueOf(text_num.getText());								
				} catch (Exception e2) {
					// TODO: handle exception
				...
				int i=JOptionPane.showConfirmDialog(null, "确定删除 "+name+"("+num+")？","提示", JOptionPane.YES_NO_OPTION);
  	  			if(i==JOptionPane.OK_OPTION){
  	  				 ...
  	  				 * ！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
  	  				 */
	  	  			Obj_Chief cai = new Obj_Chief(num, name, 0, 0);
	  				try {
						ComonManage.Chief_manage(cai, ComonManage.Delete);
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
			...
			public void mouseClicked(MouseEvent e) {
				...
				int i=JOptionPane.showConfirmDialog(null, "确定修改？","提示", JOptionPane.YES_NO_OPTION);
  	  			if(i==JOptionPane.OK_OPTION){
	  	  			...
	  	  			Obj_Chief cai = new Obj_Chief(num, name, intlevel, price);
	  				try {
						ComonManage.Chief_manage(cai, ComonManage.Update);
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
  	  				...
  	  				table.setValueAt(num, Dealrow, Define.Model_Chiefnum);
  	  				table.setValueAt(name, Dealrow, Define.Model_Chiefname);
  	  				table.setValueAt(intlevel, Dealrow, Define.Model_Chieflevel);
  	  				table.setValueAt(price, Dealrow, Define.Model_Chiefprice);
  	  				JOptionPane.showMessageDialog(null,"操作成功！","恭喜",JOptionPane. INFORMATION_MESSAGE);
  	  			}else{
  	  				System.out.println("放弃修改");
  	  			}
			...
			public void mouseClicked(MouseEvent e) {
                ...
                if (selectedRow.length != 0){//有选择
                	 ...
                	 */
              	  	...
              	  	text_num.setText(table.getValueAt(row, Define.Model_Chiefnum).toString());
              	  	text_name.setText(table.getValueAt(row, Define.Model_Chiefname).toString());
              	  	//text_level.setText(table.getValueAt(row, Define.Model_Chieflevel).toString());
              	    comboBox_level.setSelectedIndex((int) table.getValueAt(row, Define.Model_Chieflevel));
              	  	text_price.setText(table.getValueAt(row, Define.Model_Chiefprice).toString());
