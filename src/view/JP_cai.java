...
public class JP_cai extends JPanel {
	private JTextField text_num;
	private JComboBox comboBox_level;
	private JTextField text_name;
	private JTextField text_price;
	private JTable table;
	private MyAATModel model_cai;						//该滚滚滚滚滚滚滚滚	用全局替换
	private TableRowSorter sorter;
	static int Dealrow = -1;//记录表格当前操作的行
	String sqlsql;
	RetrieveObject RO = new RetrieveObject();			//该滚滚滚滚滚滚滚滚	用全局替换
	//JScrollPane scrollPane;
	/**
	 * Create the panel.
	 * 
	 * 就改了三样，model_	Obj_  Define.
	 */
	public JP_cai() {
		sqlsql = "select * from menu";		//该滚滚滚滚滚滚滚滚	用全局替换
		...
		model_cai = RO.getTableModel(Define.Cai_columnNames2, sqlsql);					//该滚滚滚滚滚滚滚滚
		sorter = new TableRowSorter(model_cai);  					//该滚滚滚滚滚滚滚滚	
		table = new JTable();
		table.setCellSelectionEnabled(false);//设置此表是否允许同时存在行选择和列选择。
		ListSelectionModel cellSelectionModel = table.getSelectionModel();
		table.setModel(model_cai);
		...
		text_price.setBounds(281, 64, 109, 30);
		...
		//drop.setContentAreaFilled(false);//透明
		panel_2.add(drop);
		...
		insert.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int getCnum;
				try {
					getCnum = ComonManage.Max_num("menu")+1;
		...
		saveinsert.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int price = 0;
				int num;
				int intlevel = Obj_Cai.Tointlevel(level);					//该滚滚滚滚滚滚滚滚
				try {	
				...
				int i=JOptionPane.showConfirmDialog(null, "确定插入？","提示", JOptionPane.YES_NO_OPTION);
  	  			if(i==JOptionPane.OK_OPTION){
  	  				 ...
  	  				 * ！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
  	  				 */
  	  				Obj_Cai cai = new Obj_Cai(num, name, intlevel, price);
  	  				try {
						ComonManage.Cai_manage(cai, ComonManage.Insert);
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
	  	  			...
	  	  			model_cai = RO.getTableModel(Define.Cai_columnNames2, sqlsql);	//这里是获得数据库最新数据
	  				sorter = new TableRowSorter(model_cai); 	//排序
	  				table.setModel(model_cai);	
	  				table.setRowSorter(sorter);
					table.setCellSelectionEnabled(false);//设置此表是否允许同时存在行选择和列选择。//该滚滚滚滚滚滚滚滚
		...
		refresh.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				 ...
				 */
					model_cai = RO.getTableModel(Define.Cai_columnNames2, sqlsql);	//这里是获得数据库最新数据
					//DefaultTableModel model_cai = Obj_Cai.getmodel();
	  				sorter = new TableRowSorter(model_cai); 	//排序
	  				table.setModel(model_cai);
	  				table.setRowSorter(sorter);
					table.setCellSelectionEnabled(false);//设置此表是否允许同时存在行选择和列选择。
		...
		drop.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int num = 0;				
				String name = (String) table.getValueAt(Dealrow, Define.Model_Cname-1);
				try {
					num = Integer.valueOf(text_num.getText());								
				} catch (Exception e2) {
					// TODO: handle exception
				...
				int i=JOptionPane.showConfirmDialog(null, "确定删除 "+name+"("+num+")？","提示", JOptionPane.YES_NO_OPTION);
  	  			if(i==JOptionPane.OK_OPTION){
  	  				 ...
  	  				 * ！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
  	  				 */
	  	  			Obj_Cai cai = new Obj_Cai(num, name, 0, 0);
		  				try {
						ComonManage.Cai_manage(cai, ComonManage.Delete);
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
  	  				...
  	  				model_cai = RO.getTableModel(Define.Cai_columnNames2, sqlsql);	//这里是获得数据库最新数据
  	  				sorter = new TableRowSorter(model_cai); 	//排序					//该滚滚滚滚滚滚滚滚
  	  				table.setModel(model_cai);
  	  				table.setCellSelectionEnabled(false);//设置此表是否允许同时存在行选择和列选择。
					table.setRowSorter(sorter); 
			...
			public void mouseClicked(MouseEvent e) {
				...
				int i=JOptionPane.showConfirmDialog(null, "确定修改？","提示", JOptionPane.YES_NO_OPTION);
  	  			if(i==JOptionPane.OK_OPTION){
	  	  			...
	  	  			Obj_Cai cai = new Obj_Cai(num, name, intlevel, price);
	  				try {
					ComonManage.Cai_manage(cai, ComonManage.Update);
					} catch (SQLException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
  	  				...
  	  				table.setValueAt(num, Dealrow, Define.Model_Cnum-1);
  	  				table.setValueAt(name, Dealrow, Define.Model_Cname-1);
  	  				table.setValueAt(intlevel, Dealrow, Define.Model_Clevel-1);
  	  				table.setValueAt(price, Dealrow, Define.Model_Cprice-1);
  	  				JOptionPane.showMessageDialog(null,"操作成功！","恭喜",JOptionPane. INFORMATION_MESSAGE);
  	  			}else{
  	  				System.out.println("放弃修改");
  	  			}
			...
			public void mouseClicked(MouseEvent e) {
                ...
                if (selectedRow.length != 0){//有选择
                	 ...
                	 */
              	  	...
              	  	text_num.setText(table.getValueAt(row, Define.Model_Cnum-1).toString());
              	  	text_name.setText(table.getValueAt(row, Define.Model_Cname-1).toString());
              	  	//text_level.setText(table.getValueAt(row, Define.Model_Clevel).toString());
					//该滚滚滚滚滚滚滚滚
              	    comboBox_level.setSelectedIndex((int) table.getValueAt(row, Define.Model_Clevel-1) );
              	  	text_price.setText(table.getValueAt(row, Define.Model_Cprice-1).toString());
